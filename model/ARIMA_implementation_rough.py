...
def ARIMA_predict():
    ...
    diffDays = pd.date_range(data[0][-1], datetime.datetime.today(),
                             freq=BDay()).size - 1

    # predict the missing number of days and then the current day
    # so that the prediction is for tomorrow not
