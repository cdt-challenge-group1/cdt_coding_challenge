...
from headlines import collect_and_analyse_headlines
...
if __name__ == "__main__":
    headlines = collect_and_analyse_headlines()

    prices, predicted_stdevs = ARIMA_predict()
    print('Price today:', prices[1][-2], 'Predicted price tomorrow:',
                                         prices[1][-1])
    price_today = prices[1][-2]
    price_prediction = prices[1][-1]

    # For now, we check only 1 day ahead, so if today's price is higher
    # then dock today
    if price_today >= price_prediction:
        dock_today = True
    # If the prediction is higher,
    # check whether the delay cost is worth it
    elif (price_prediction * NUM_BARRELS) - DELAY_COST > \
         (price_today * NUM_BARRELS):
        dock_today = False
    else:
        dock_today = True
    ...
    textout = '\n'.join((r"Today's price=%.2f" % price_today,
    ...
    plt.errorbar([x.strftime("%Y-%m-%d") for x in xs[-10:]], ys[-10:],
                 yerr=errors[-10:],
                 elinewidth=2, uplims=True,
                 lolims=True, ecolor="red")
    plt.xticks(rotation=70)
    plt.ylabel('Price (USD/Barrel)')
    plt.xlabel('Date')
    plt.title('Predicted Oil Price')
    plt.text(([x.strftime("%Y-%m-%d") for x in xs[-10:]][-2]),
             (price_prediction+3), textout)
    plt.tight_layout()
