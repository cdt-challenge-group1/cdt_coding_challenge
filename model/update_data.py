...
def updateData():
    ...
    diff_days = pd.date_range(data[0][-1], datetime.datetime.today(),
                              freq=BDay()).size

    if diff_days - 1 > 0:
        # need to trim the 1st value from yahoo since it
        # includes the current day too
        yahoo_data = yahoo.get_yahoo_data(diff_days)
        yahoo_data = (yahoo_data[0][1:], yahoo_data[1][1:])
